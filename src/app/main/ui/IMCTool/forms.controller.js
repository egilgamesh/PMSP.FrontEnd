(function () {
  ...
  // angular
  //   .module('app.ui.forms.IMC').config(function($mdDateLocaleProvider) {
  ...
  angular
    ...
    .controller('IMCFormsController', FormsController);
  ...
  function FormsController(api, $scope, NewFFSGeneralStorage, $stateParams, $mdDialog, $state, appConfig, $http) {
    ...
    loadData();
    function loadData() {
      api.IMC.ByID.get({'id': $stateParams.id},
    ...
    vm.isModuleIsActive = function(fields) {
      ...
      angular.forEach(fields['Components'], function(fieldRefine, something) {

        angular.forEach(fieldRefine["Fields"], function(obj, values){
    ...
    vm.COROptions = function (event, status) {
      $mdDialog.show({
        ...
        focusOnOpen: false,
        targetEvent: event,
        locals: { items: {id: vm.id, status: status, form: 'IMC' }},
