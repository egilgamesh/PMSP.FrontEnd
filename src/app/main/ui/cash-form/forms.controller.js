(function ()
{
  ...
  angular
    .module('app.ui.forms.cash').factory('cashStorage', function () {
    var STORAGE_ID = 'cashForm';
  ...
  angular
    ...
    .controller('CashFormsController', FormsController);


  /** @ngInject */
  function FormsController(api, $scope, cashStorage, $stateParams, $mdDialog, $state, appConfig, FFSConstant)
  {
    var vm = this;
    ...
    function loadData() {
      api.ffs.cashById.get({'id': $stateParams.id},
        ...
        function (response)
        {
          ...
          vm.cashForm.DateOfSiteVisit = new Date(response.data.DateOfSiteVisit);
        },
    ...
    if (vm.id){
      ...
      var cashDataTemp = cashStorage.get();
      if(cashDataTemp.DateOfSiteVisit){
        cashDataTemp.DateOfSiteVisit = new Date(cashDataTemp.DateOfSiteVisit);
      }

      vm.cashForm = angular.copy(cashDataTemp);
      $scope.$watch('vm.cashForm', function (newVal, oldVal) {
        cashStorage.put(vm.cashForm);
      }, true);
    ...
    function sendForm(ev) {
      ...
      if(submitData.DateOfSiteVisit){
        submitData.DateOfSiteVisit = moment(vm.cashForm.DateOfSiteVisit).format('Y-MM-DD');
      }


      api.ffs.cash.save(submitData,
    ...
    vm.COROptions = function (event, status) {
      $mdDialog.show({
        ...
        focusOnOpen: false,
        targetEvent: event,
        locals: { items: {id: vm.id, status: status, form: 'Cash' }},
