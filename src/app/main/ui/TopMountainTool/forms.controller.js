(function () {
  ...
  // angular
  //   .module('app.ui.forms.TopMountain').config(function($mdDateLocaleProvider) {
  ...
  angular
    ...
    .controller('TopMountainFormsController', FormsController);
  ...
  function FormsController(api, $scope, NewFFSGeneralStorage, $stateParams, $mdDialog, $state, appConfig, $http) {
    ...
    loadData();
    function loadData() {
      api.TopMountain.ByID.get({'id': $stateParams.id},
    ...
    vm.COROptions = function (event, status) {
      $mdDialog.show({
        ...
        focusOnOpen: false,
        targetEvent: event,
        locals: { items: {id: vm.id, status: status, form: 'TopMountain' }},
    ...
    function sendForm(ev) {
      ...
      api.TopMountain.General.save(data,
