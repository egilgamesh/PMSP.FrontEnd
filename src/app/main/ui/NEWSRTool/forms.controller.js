(function () {
  ...
  // angular
  //   .module('app.ui.forms.NEWSR').config(function($mdDateLocaleProvider) {
  ...
  angular
    ...
    .controller('NEWSRFormsController', FormsController);
  ...
  function FormsController(api, $scope, NewFFSGeneralStorage, $stateParams, $mdDialog, $state, appConfig, $http) {
    ...
    loadData();
    function loadData() {
      api.NEWSR.ByID.get({'id': $stateParams.id},
    ...
    vm.COROptions = function (event, status) {
      $mdDialog.show({
        ...
        focusOnOpen: false,
        targetEvent: event,
        locals: { items: {id: vm.id, status: status, form: 'NEWSR' }},
