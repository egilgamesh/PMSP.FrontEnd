(function ()
{
  ...
  angular.module('app.components.tables.IGPA')
        .controller('IGPAController', MainController);

    /** @ngInject */
    function MainController($http, $scope, $IGPAFormAPI, $IGPASearchFormAPI, $mdDialog, appConfig, $state, $stateParams, $timeout, $q, $log)
    {
      var folderPath = 'app/main/components/tables/igpa-table/';
      var vm = this;
      ...
      $http.get(appConfig.apiUrl + 'IGPAFiltersItems').then(function (response) {
      ...
      vm.addItem = function (event) {
        console.log(event);
        $state.go('app.ui_forms-IGPATool');
      };

      vm.delete = function (event) {
        $mdDialog.show({
          clickOutsideToClose: true,
          controller: 'DelIGPAAPIController',
          controllerAs: 'ctrl',
          focusOnOpen: false,
          targetEvent: event,
      ...
      vm.getItems= function () {
        ...
        vm.promise = $IGPAFormAPI.items.get(vm.query, success).$promise;
      };
      ...
      function querySearch (query) {
        ...
        $http.get(appConfig.apiUrl + 'SearchIGPAForm/' + finalQuery)
